    ...
    <section className="paper-grid bg-ivory px-6 py-24 sm:px-12">
      <div className="mx-auto max-w-6xl">
        <div className="mb-12 max-w-2xl">
          ...
          <h2 className="display-heading mt-3 text-2xl text-ink sm:text-3xl">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {morePerspectives.map((project) => (
            <ScrollReveal key={project.title}>
              <article className="flex h-full flex-col gap-4 rounded-2xl border border-ink/10 bg-ivory/70 p-7 shadow-sm">
                <div>
                ...
                {project.link && (
                  <a
                    ...
                    data-cursor="link"
                    className="inline-flex w-fit items-center gap-2 rounded-full border border-ink/10 bg-ink/[0.03] px-4 py-2 text-xs font-semibold text-ink transition-colors hover:bg-ink/[0.06]"
                  >
                    {project.link.label === "Instagram" ? (
                      <svg
                        aria-hidden
                        viewBox="0 0 24 24"
                        className="h-3.5 w-3.5 text-pink"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth={2}
                      >
                    ...
                    {project.link.handle ?? `${project.link.label} 보기`}
                  </a>
                )}
              </article>
            </ScrollReveal>
